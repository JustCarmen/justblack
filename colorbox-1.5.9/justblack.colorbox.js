	...
	if(useGviewer === 1) {
		jQuery("a[type$=pdf].gallery").colorbox({
			...
			html:		function(){
							...
							return '<iframe width="100%" height="100%" src="http://docs.google.com/viewer?url=' + WT_SERVER_NAME + WT_SCRIPT_PATH + JB_COLORBOX_URL + 'pdfviewer.php?mid=' + mid + '&embedded=true"></iframe>';
						},
			onComplete: function() {
						longTitles();
						if(useWatermark === 1) {
